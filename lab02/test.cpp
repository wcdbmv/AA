...
void test(size_t begin, size_t end, size_t step) {
	std::cout << "size             product       bad::winograd      good::winograd\n";
	for (size_t i = begin; i <= end; i += step) {
		...
		std::cout
			...
			<< std::setw(20) << count_time(product, m1, m2)
			<< std::setw(20) << count_time(bad::coppersmith_winograd_product, m1, m2)
			<< std::setw(20) << count_time(good::coppersmith_winograd_product, m1, m2) << '\n';
	}
	std::cout << '\n';
}
